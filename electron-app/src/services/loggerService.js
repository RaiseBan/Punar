...
const TEMPLATES = {
    timestamp: (ts) => `${COLORS.DIM}[${ts}]${COLORS.RESET}`,
    level: (level) => `${COLORS.LEVEL[level]}[${level}]${COLORS.RESET}`,
    module: (module) => `${COLORS.MODULE[module]}[${module}]${COLORS.RESET}`,
    success: (msg) => `${COLORS.BRIGHT_GREEN}✓ ${msg}${COLORS.RESET}`,
    failure: (msg) => `${COLORS.BRIGHT_RED}✗ ${msg}${COLORS.RESET}`,
    highlight: (msg) => `${COLORS.BOLD}${msg}${COLORS.RESET}`,
    value: (val) => `${COLORS.BRIGHT_CYAN}${val}${COLORS.RESET}`
};

class LoggerService {
    constructor() {
        this.LOG_LEVELS = LOG_LEVELS;
        this.LOG_MODULES = LOG_MODULES;
     ...
     * @param {Object} [data] - Дополнительные данные
     */
    failure(module, message, data = null) {
