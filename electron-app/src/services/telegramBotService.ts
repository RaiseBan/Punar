...
class TelegramBotService {
    ...
    groupToTasksMap: Record<string, Array<number | string>>;
    messageQueue: QueuedMessage[];
    processing: boolean;
    processStatusTracking: Map<string, ProcessStatus>;
    ...
    loadConfig(): void {
        try {
            ...
            if (fs.existsSync(configPath)) {
                const config = JSON.parse(fs.readFileSync(configPath, 'utf8')) as BotConfig;
    ...
    async setBotToken(token: string): Promise<StatusResult> {
        this.stopPolling();
    ...
    addChatId(chatId: number | string): void {
        ...
        if (!this.chatIds.includes(numericChatId)) {
            this.chatIds.push(numericChatId);
            this.saveConfig();
        }
    }

    removeChatId(chatId: number): void {
    ...
    async sendMessage(chatId: number, text: string, options: MessageOptions = {}): Promise<any> {
    ...
    async editMessageReplyMarkup(chatId: number, messageId: number, replyMarkup: ReplyMarkup): Promise<any> {
        if (!this.botToken) return;
    ...
    async answerCallbackQuery(callbackQueryId: string, text: string | null = null, showAlert: boolean = false): Promise<any> {
        if (!this.botToken) return;
        ...
        try {
            ...
            if (message.includes('Процесс остановлен вручную') || message.includes('Задача ID:')) {
                ...
                if (taskId) {
                    ...
                    this.processStatusTracking.set(statusKey, {
    ...
    addMessageToQueue(messageData: QueuedMessage): void {
                ...
                if (this.chatIds && this.chatIds.length > 0) {
                    for (const chatId of this.chatIds) {
                        try {
                            ...
                            console.error(`[TG Bot Queue] Ошибка при отправке уведомления в чат ${chatId}:`, error);
                        }
                    }
                }
            }
     ...
     */
    async handleIncomingMessage(chatId: number, message: any): Promise<void> {
        try {
                        ...
                        if (helpText.length > 3000) {
                            ...
                            for (let i = 0; i < chunks.length; i++) {
                                try {
                                    ...
                                    console.error(`[TG Bot] Ошибка при отправке части ${i + 1} справки: ${(error as Error).message}`);
                                }
                            }
                        } else {
                            // Отправляем в текстовом формате для избежания проблем с HTML-тегами
            ...
            console.error(`[TG Bot] Ошибка при обработке сообщения: ${(error as Error).message}`);
            try {
                await this.sendMessage(chatId, `❌ Произошла ошибка при обработке вашего сообщения: ${(error as Error).message}`);
            } catch (sendError) {
                console.error(`[TG Bot] Не удалось отправить сообщение об ошибке: ${(sendError as Error).message}`);
            }
        }
    }
    ...
    async getTasks(): Promise<Task[]> {
        return new Promise((resolve, reject) => {
            try {
                ...
                const responseListener = (event: Electron.IpcMainEvent, tasks: Task[]) => {
    ...
    async handleTasksCommand(chatId: number): Promise<void> {
        try {
            ...
            const groupToTasksMap: Record<string, GroupTask> = {};
            let groupCounter = 0;
            ...
            for (const [groupKey, tasksInGroup] of Object.entries(groupedTasks)) {
                ...
                let inlineKeyboard: Array<Array<InlineKeyboardButton>> = [];
            ...
            await this.sendMessage(chatId, 'Произошла ошибка при получении списка задач: ' + (error as Error).message);
        }
    }
    ...
    registerCommand(command: string, handler: any): boolean {
     ...
     * @param maxLength - Максимальная длина части (по умолчанию 3000)
     * @returns - Массив частей текста
     */
    splitIntoChunks(text: string, maxLength: number = 3000): string[] {
        if (!text) return [];
     ...
     * @param mevLoadBalancer - Экземпляр класса MevLoadBalancer
     */
    initMevCommands(mevLoadBalancer: any): void {
...
**Оставшиеся процессы перераспределены**`);
    ...
    async sendTaskStatus(taskId: string | number): Promise<boolean> {
        try {
            ...
            const task = tasks.find(t => t.id === parseInt(String(taskId)));
...
ipcMain.handle('telegram-bot:set-token', (event, token: string) => {
...
ipcMain.handle('telegram-bot:send-task', (event, taskData: TaskData) => {
...
ipcMain.handle('telegram-bot:send-task-status', (event, taskId: string | number) => {
