...
const mevSubtaskProcesses = new Map<string | number, ProcessInfo>();
...
async function forceKillWindowsProcess(pid: number): Promise<boolean> {
    if (!pid) {
    ...
    try {
        ...
        try {
            await new Promise<void>((resolve, reject) => {
                treeKill(pid, 'SIGKILL', (err: Error | null) => {
                    if (err) {
            ...
            console.warn(`⚠️ KILL: Не удалось использовать tree-kill: ${(treeKillError as Error).message}`);
        }
        ...
        try {
            ...
            console.error(`❌ KILL: Не удалось завершить процесс ${pid} с помощью taskkill: ${(taskKillError as Error).message}`);
            return false;
        }
    } catch (error) {
        console.error(`❌ KILL: Критическая ошибка при попытке завершения процесса ${pid}: ${(error as Error).message}`);
        return false;
    }
}
...
async function findBestMeteoraPool(tokenAddress: string, currentPairAddress: string): Promise<string | null> {
    try {
        ...
        const meteoraPairs = response.data.pairs.filter((pair: any) => pair.dexId === "meteora");
        ...
        console.error(`МОНИТОРИНГ: Ошибка при проверке DexScreener API: ${(error as Error).message}`);
        return null;
    }
}
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            if (mevSubtaskProcesses.has(taskId)) {
                ...
                if (processInfo && processInfo.intervalId) {
...
async function spawnProcess(taskConfig: any, userSettings: any): Promise<any | null> {
    ...
    try {
        ...
        if (moduleDir === "mev") {
            ...
            child = spawn(exePath, ["--port", userSettings.token_release_port || "8000"], {
                stdio: "pipe",
                shell: true,
                detached: false,
...
`;
            ...
            if (updatedTaskConfig.enablePoolMonitoring === true) {
                ...
                const tokenAddress = updatedTaskConfig.rowData && updatedTaskConfig.rowData[0] ?
                    updatedTaskConfig.rowData[0] :
                    updatedTaskConfig.tokenAddress;

                if (!tokenAddress) {
                    console.error(`МОНИТОРИНГ: Не удалось определить адрес токена для задачи ${taskId}`);
                    return child;
                }
                ...
                const intervalId = setInterval(async () => {
                    try {
                        ...
                        if (currentMeteoraPair) {
                            ...
                            if (betterPairAddress) {
                                ...
                                const newConfigFilePath = await generateMevConfig(
                                    userSettings.mevBotDirectory as string,
                                    path.join(pythonScriptPath, "tokens"),
                                    updatedTaskConfig,
                                ...
                                newChild.on('exit', (code: number | null) => {
                                    clearTimeout(startupTimeout);
                        ...
                        console.error(`МОНИТОРИНГ: Ошибка в интервале мониторинга для задачи ${taskId}: ${(error as Error).message}`);
                    }
                }, checkInterval);
        ...
        const processCheckInterval = setInterval(() => {
            ...
            try {
                ...
                console.error(`ПРОЦЕСС ${taskId}: Ошибка при проверке состояния WSL процесса:`, (e as Error).message);
