...
export default function WithdrawBalances({
    ...
    const [fromMethod, setFromMethod] = useState<"existing" | "manual" | "set">("existing");
    const [selectedKey, setSelectedKey] = useState("");
    const [manualKey, setManualKey] = useState("");
    const [setName, setSetName] = useState("");
    const [toAddress, setToAddress] = useState("");
    const [toMethod, setToMethod] = useState<"existing" | "manual">("existing");
    ...
    const handleWithdraw = async () => {
        ...
        try {
            if (fromMethod === "existing") {
                ...
                senders = set.map((w: any) => Keypair.fromSecretKey(bs58.decode(w.privateKey)));
            }
        } catch (e) {
        ...
        try {
            ...
            for (const sender of senders) {
                try {
                    ...
                    console.log(`account size: ${(await connection.getAccountInfo(sender.publicKey))?.data.byteLength}`);
        ...
        <Box sx={{
            ...
            <Box sx={{ mb: 3 }}>
                ...
                <RadioGroup
                    row
                    value={fromMethod}
                    onChange={(e) => setFromMethod(e.target.value as any)}
                    sx={{ gap: 3, mb: 2 }}
                >
                    <FormControlLabel
                        value="existing"
            ...
            <Box sx={{ mt: 4 }}>
                ...
                <RadioGroup
                    row
                    value={toMethod}
                    onChange={(e) => setToMethod(e.target.value as any)}
                    sx={{ gap: 3, mb: 2 }}
                >
                    <FormControlLabel
                        value="existing"
