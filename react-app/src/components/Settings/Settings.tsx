...
export default function Settings() {
  ...
  const validateField = useCallback(
    (field: keyof AppSettings, value: string | undefined): string | null => {
      ...
      // API Tokens
      if (
        field === 'tensor_api_token' ||
        field === 'bloxroute_api_token' ||
        field === 'thor_streamer_token'
      ) {
        if (!value) return null; // Опционально
      ...
      if (
        field === 'lookupOwner' ||
        field === 'migration_wallet' ||
        field === 'thor_streamer_address'
      ) {
        if (!value) return null; // Опционально
      ...
      // Ports
      if (
        field === 'proxy_server_port' ||
        field === 'token_release_port' ||
  ...
  const handleBlur = useCallback(
    (field: keyof AppSettings) => {
      ...
      const error = validateField(field, value !== undefined ? String(value) : undefined);
  ...
  const handleHeliusRpcChange = (index: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (touched.has(fieldKey)) {
      ...
      setErrors((prev) => ({
        ...prev,
        [fieldKey]: error.isValid ? null : error.error || 'Invalid URL',
      }));
    }
  };
        ...
        <Stack spacing={2}>
          {settings.heliusRpcs?.map((rpc, index) => (
            <Box key={index} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
              ...
              <IconButton color="error" onClick={() => handleDeleteHeliusRpc(index)} sx={{ mt: 2 }}>
                <DeleteIcon />
              </IconButton>
            </Box>
          ))}
