...
type TxTypeChipColor = ChipProps['color'];
...
const getTxTypeColor = (txType: string): TxTypeChipColor => {
  switch (txType) {
    case TX_TYPES.LIST:
      return 'success';
...
const TxHistorySearch = () => {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      while (collected.length < filters.txCount && attempts < MAX_ATTEMPTS) {
        ...
        const filtered = response.txs.filter(
          (tx) =>
            tx.mint?.name &&
            (!filters.name || tx.mint.name.toLowerCase().includes(filters.name.toLowerCase()))
        );
  ...
  const formatAddress = (address?: string) => (address ? `${address.slice(0, 5)}` : '');
      ...
      if (viewMode === VIEW_MODES.CARDS || viewMode === VIEW_MODES.MEDIUM) {
        return Array.from({ length: filters.txCount }).map((_, index) => (
          <Grid
            item
            xs={12}
            sm={viewMode === VIEW_MODES.CARDS ? 6 : 12}
            md={viewMode === VIEW_MODES.CARDS ? 4 : 6}
            key={`skeleton-${index}`}
          >
        ...
        <CardContent>
          <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={1}>
            ...
            <Chip
              ...
              color={getTxTypeColor(tx.tx.txType)}
              size="small"
              sx={{ fontWeight: 'bold' }}
            />
          </Box>
              ...
              {tx.tx.buyerId && (
                <>
                  ...
                  <Tooltip title={tx.tx.buyerId}>
                    <Chip size="small" label={formatAddress(tx.tx.buyerId)} sx={{ mt: 0.5 }} />
                  </Tooltip>
                </>
              )}
            </Typography>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
            ...
            >
              ...
              <Typography variant="caption">{tx.tx.txId.slice(0, 8)}...</Typography>
            ...
            <Tooltip title={tx.mint.onchainId}>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Typography variant="caption">{tx.mint.onchainId.slice(0, 6)}...</Typography>
              </Box>
            </Tooltip>
          </Box>
        </CardContent>
          ...
          >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, minWidth: 0 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
                <MonetizationOnIcon
                  fontSize="small"
                  sx={{ mr: 0.5, fontSize: 16, color: theme.palette.primary.main }}
                />
        ...
        >
          ...
          {isExpanded && (
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <CalendarTodayIcon fontSize="small" color="primary" sx={{ mr: 1 }} />
                <Typography variant="body2">{formatDate(tx.tx.txAt)}</Typography>
              </Box>

              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <LayersIcon fontSize="small" color="primary" sx={{ mr: 1 }} />
                <Typography variant="body2">Block: {tx.tx.blockNumber}</Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                >
                  <ReceiptIcon fontSize="small" sx={{ mr: 0.5 }} />
                  <Typography variant="caption">TX: {tx.tx.txId.slice(0, 8)}...</Typography>
                  <OpenInNewIcon fontSize="small" sx={{ ml: 0.5, fontSize: 12 }} />
                </Link>
              </Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gridColumn: '1 / -1' }}>
                ...
                <Typography variant="caption">NFT ID: {tx.mint.onchainId}</Typography>
              </Box>
            </Box>
          )}
        </Paper>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={3}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', width: 300 }}>
            <Box sx={{ mr: 2, display: 'flex', alignItems: 'center' }}>
              <ViewListIcon
                sx={{ color: viewMode <= VIEW_MODES.COMPACT ? 'primary.main' : 'text.secondary' }}
              />
            </Box>
            <Slider
              value={viewMode}
            ...
            <Box sx={{ ml: 2, display: 'flex', alignItems: 'center' }}>
              <ViewModuleIcon
                sx={{ color: viewMode >= VIEW_MODES.MEDIUM ? 'primary.main' : 'text.secondary' }}
              />
            </Box>
          </Box>
        </Box>
            ...
            <Grid item xs={12}>
              <Paper
                ...
                sx={{
                  ...
                  borderRadius: 2,
                  background:
                    theme.palette.mode === 'dark' ? 'rgba(38,50,56,0.4)' : 'rgba(236,239,241,0.6)',
                }}
              >
