...
const TENSOR_GET_INVENTORY_ENDPOINT = "https://api.mainnet.tensordev.io/api/v1/user/inventory_by_collection";
...
const Statistic: React.FC = () => {
    ...
    useEffect(() => {
        const loadPersistedData = async () => {
            ...
            if (savedParams) {
                const { wallet: sWallet, collection: sCollection, search: sSearch } = JSON.parse(savedParams);
                setWallet(sWallet);
    ...
    const fetchCurrentPrice = useCallback(async (name: string, collId: string) => {
        const params = new URLSearchParams({
            collId,
